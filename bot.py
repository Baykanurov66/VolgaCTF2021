...
S=InlineKeyboardButton('Наш сайт🌐', url='https://volgactf.ru/volgactf-2021/final/')
...
I=InlineKeyboardButton('Мы в Инстраграм💬', url='https://instagram.com/volga_ctf?utm_medium=copy_link')
kontakt=InlineKeyboardMarkup(row_width=2).add(S,V,T,G,Y,I)

S1=InlineKeyboardButton('Our website🌐', url='https://volgactf.ru/volgactf-2021/final/')
V1=InlineKeyboardButton('We are on VKontakte🔷', url='https://vk.com/volgactf')
T1=InlineKeyboardButton('We are on Twitter📖', url='https://twitter.com/volgactf')
G1=InlineKeyboardButton('Our Github💠', url='https://github.com/volgactf')
Y1=InlineKeyboardButton('We are on Youtube🛑', url='https://www.youtube.com/channel/UCTbjY5Xys-CgMBN8jgLazPA')
I1=InlineKeyboardButton('We are on Instagram💬', url='https://instagram.com/volga_ctf?utm_medium=copy_link')
...
async def process_start_command(msg: types.Message, state: FSMContext):
    ...
    if msg.from_user.id not in i:
        ...
        if str(*lan1) == "ru":
            ...
            await msg.answer("<b>We already met! </b> \nIf you want to remember what I can do, we will execute the command /help", parse_mode='HTML')
    c.close()
    db.close()
...
async def process_callback_button1(callback_query: types.CallbackQuery):
    ...
    if callback_query.from_user.id not in i:
        ...
        db.commit()
        await bot.send_photo(callback_query.from_user.id, 'AgACAgIAAxkBAAICj2E8Xrp-Qmybi1e0hvLu18e6g5JpAAIEtDEbCXrhScSNA-9glO0NAQADAgADeQADIAQ',caption = "<b>\nWelcome to VolgaCTF 2021 </b>"+name+"!\n\nI am a <b> bot volunteer </b> who will help you navigate these competitions.\n\nI will remind you of the lectures and other events that will take place during VolgaCTF 2021.\n\n🟡 You can find out about all my capabilities by calling the command /menu\n⚫️ If you forget something, I will always be happy to remind you as a team /help\n🟠 You can view our contacts by the team /kontakt\n\nNow let's decide on your status at this event, choose one option from the proposed list:", reply_markup=Vhod1, parse_mode='HTML')
    else:
...
async def gamer(msg: types.Message, state: FSMContext):
    ...
    if str(*lan) == "ru":
        ...
        await bot.send_photo(msg.from_user.id, "AgACAgIAAxkBAAIEmWE9wHto7DbRtSr-OEmurheorrioAALStTEbCXrxSU3P6kjBcZIhAQADAgADeAADIAQ" , caption = "Fine!\nHave a good competition❤️\nAnd remember: Power is safe!\n\nOur contacts:", reply_markup=kontakt1, parse_mode='HTML')
    await state.finish()
    c.close()
    db.close()
...
async def guest(callback_query: types.CallbackQuery):
    ...
    if callback_query.from_user.id not in i:
        ...
        if str(*lan) == "ru":
            ...
            await bot.send_photo(callback_query.from_user.id, "AgACAgIAAxkBAAIEmWE9wHto7DbRtSr-OEmurheorrioAALStTEbCXrxSU3P6kjBcZIhAQADAgADeAADIAQ" , caption = "Fine!\nHave a good competition❤️\nAnd remember: Power is safe!\n\nOur contacts:", reply_markup=kontakt1, parse_mode='HTML')
    else:
        await bot.send_message(callback_query.from_user.id,"Ты уже выбрал свой статус!!\n\nПоэтому всё всё....")
    c.close()
    db.close()
...
async def process_help_command(msg: types.Message):
    ...
    if msg.from_user.id not in i:
        ...
        if str(*lan) == "ru":
            ...
            await bot.send_photo(msg.from_user.id, "AgACAgIAAxkBAAIEgWE9vm3lCZdVy-BrSA8btxBMj2HpAALNtTEbCXrxSc3jZYo4m6eYAQADAgADeQADIAQ" , caption = "⚠️During the entire time of the competition, I will remind you of lectures and other events\n\n🟡To open the options menu, run the command /menu\n\n🟨You can always find out about us and see our contacts using the command /kontakt\n\n🔆This is all my powers!\nIf you have something to offer or how to improve my work, feel free to ask the organizers!", reply_markup=kom1, parse_mode='HTML')
...
async def process_kontakt_command(msg: types.Message):
    ...
    if msg.from_user.id not in i:
        ...
        if str(*lan) == "ru":
            ...
            await bot.send_photo(msg.from_user.id, "AgACAgIAAxkBAAIEmWE9wHto7DbRtSr-OEmurheorrioAALStTEbCXrxSU3P6kjBcZIhAQADAgADeAADIAQ" , caption = "Our contacts are wherever we are!", reply_markup=kontakt1, parse_mode='HTML')
...
async def text(msg: types.Message):
    ...
    if msg.from_user.id not in i and msg.from_user.id not in o:
        c.execute('SELECT lang FROM Language WHERE id = ?',(msg.from_user.id, ))
        lan=c.fetchone()
        if str(*lan) == "ru":
            await msg.answer("<b>Выполни для начала команду /start и укажи свой статус с помощью кнопок</b>", parse_mode='HTML')
        ...
        if str(*lan) == "ru":
            ...
            await bot.send_photo(msg.from_user.id, "AgACAgIAAxkBAAIEb2E9uxwY971J9yzP_V0wAAFoz85SmwACyrUxGwl68Ul77sScxHKKjQEAAwIAA3kAAyAE" , caption = "🔶 You can ask a question <b>organizers</b> or <b>to the speaker</b>, who is leading the lecture!\n\n🔶 Choose one of the menu items and write your question!", reply_markup=kom1, parse_mode='HTML')
...
async def process_callback_button1(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if callback_query.from_user.id not in i and callback_query.from_user.id not in o:
        ...
        if str(*lan) == "ru":
            ...
            await bot.send_message(callback_query.from_user.id, 'Write your question to the organizers')
        await Obr.org.set()
...
async def org(msg: types.Message, state: FSMContext):
    ...
    if str(*lan) == "ru":
        ...
        await msg.answer("The question has been sent to the organizers, soon a volunteer will definitely come to you and answer")
...
async def process_callback_button1(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if callback_query.from_user.id not in i:
        ...
        if str(*lan) == "ru":
            ...
            await bot.send_message(callback_query.from_user.id, 'Write your question for the speaker')
        await Obr.que.set()
...
async def org(msg: types.Message, state: FSMContext):
    ...
    if str(*lan) == "ru":
        ...
        await msg.answer("Thank you for your question, after the lecture the speaker will answer your question")
    p = [j[0] for j in c.execute("SELECT kom FROM Users")]
    i1 = [j[0] for j in c.execute("SELECT id FROM Users")]
    for i in range(0,len(orgi),1):
...
async def time_func():
    ...
    for i in range(1,int(lens)+1,1):
        if current_datetime.day == int(values['values'][i][1]):
            if current_datetime.hour == int(values['values'][i][2]):
